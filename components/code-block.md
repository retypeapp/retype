...
Code blocks can be grouped using a [Tab](/components/tab.md) component that lets you combine multiple code blocks into a tabbed interface. Each code block becomes a separate tab that users can switch between.
...
To exclude code blocks from the search index, set [`excludeCode`](/configuration/project.md#search-excludecode) to `true` in your project `retype.yml` configuration file.
