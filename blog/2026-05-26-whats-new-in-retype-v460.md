...
This release removes the page limit for Retype Free, introduces partial project configuration files, adds a Table Copy button, extends [Card](/components/card.md) components with `target` support, makes `[!file]` downloads smarter, and ships one of the largest performance passes in recent releases.
