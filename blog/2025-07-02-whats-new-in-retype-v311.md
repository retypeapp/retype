...
See the full [Changelog](/changelog.md) and [Feature Log](/feature-log.md) for a detailed list of updates in the `v3.11` release.
...
Set up custom themes in your `retype.yml` file starting with the Project [`theme`](/configuration/project.md#theme) setting and adding [theme variables](/configuration/theme-variables.md):
...
The Badge configuration is added directly to the [Page](/configuration/page.md#nav-badge) settings, ensuring your navigation badges stay with their content and won't get lost in separate configuration files.
...
Navigation badges support all the same variants and configuration options as regular [badge](/components/badge.md) components, including `corners`, `size`, `icon`, and **you can even configure the Badge to `link` to a different internal or external location**. The following demonstrates linking to an external website:
...
A new `base` variant is now available for [Badge](/components/badge.md), [Button](/components/button.md), and [Callout](/components/callout.md) components. This variant uses your project's [base color](/guides/themes.md) to create consistent styling that automatically adapts to your brand colors.
