...
Retype `v4.3` brings a set of new features and a solid round of enhancements. Header navigation gets dropdown sub-menu support, backlinks gain fine-grained `include` and `exclude` configuration, and date formatting becomes fully configurable via `locale.dateFormat`.
...
The dropdown menu width and horizontal offsets are customizable using the CSS variables:

- [x] `header-links-dropdown-max-width`
- [x] `header-links-dropdown-offset`
- [x] `header-links-dropdown-gap`
...
A new [`dateFormat`](/configuration/project.md#locale) setting in the [`locale`](/configuration/project.md#locale) project setting configures how dates are rendered across the site, including the date displayed as the published date and the [Last updated](/components/last-updated.md) date.
...
Retype now auto-detects the dimensions of the `branding.lgoo` automatically adds the `width` and `height` attributes to the image.
...
A pile of smaller tweaks and adjustments have been made in `v4.3` too! Everything is listed in the [Changelog](/changelog.md#v430), but here are a few of the more notable upgrades:
