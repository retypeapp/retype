...
In the following samples, replace `[version-or-tag]` with the actual version number or tag. For instance, to specifically install the version `4.5.0-preview5` release of Retype using NPM, run the command `npm install retypeapp@4.5.0-preview5 --global`.
