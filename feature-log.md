...
For a complete list of changes in each release, see the [Changelog](/changelog.md).
...
[Backlinks](/components/backlinks.md) component
...
: Full customization of your site's visual appearance through [Theme Variables](/configuration/theme-variables.md) for both light and dark modes. Override colors, typography, and other design tokens via the `theme` project setting.
...
: [Card](/components/card.md) components now support `target`, matching Button and Badge behavior. Authors can open Card links in a new tab with `target="blank"` across all Card layouts, including `signal` and `snap`, see [Card docs](/components/card.md).
...
Collapsible [Callout](/components/callout.md) component
: Callouts now support expand and collapse using the `!!-` opening syntax. The collapsed Callout behaves the same as the [Panel](/components/panel.md) component, with a clickable title and arrow toggle on the right side of the header.
...
`compact` layout for [Card](/components/card.md) component
...
[Card](/components/card.md) component
...
[Steps](/components/steps.md) component
...
Question variant for [Button](/components/button.md) and [Badge](/components/badge.md)
...
Custom themes
: Comprehensive [theme variables](/configuration/theme-variables.md) customization for both [light](/configuration/project.md#theme-base) and [dark](/configuration/project.md#theme-dark) modes, see [docs](/configuration/project.md#theme), [Themes](/guides/themes.md) guide, and [Theme Variables](/configuration/theme-variables.md).
...
: Shorthand syntax for [Button](/components/button.md), [Badge](/components/badge.md), and [Navigation Badge](/configuration/page.md#nav-badge) components using `text|variant` format. Example: `[!badge NEW|info]` instead of `[!badge text="NEW" variant"info"]`.
...
Base variant
: New `base` variant for [Button](/components/button.md), [Badge](/components/badge.md), and [Callout](/components/callout.md) components. Provides consistent styling with the site's base color theme.
...
: Retype Pro Community Key unlocks all Pro features for GitHub Pages projects. No cost, no strings attached for `*.github.io` domains, with up to 1000 pages with all [Pro](/pro/pro.md) features included, see [blog post](/blog/2025-06-06-new-gitHub-pages-community-key.md) and [GitHub Pages](/hosting/github-pages.md) setup.

Hidden comments component
: Add hidden comments to markdown that won't appear in the final website. Supports both inline `` and block comment syntax, see [docs](/components/comments.md).
...
Question [Callout](/components/callout.md)
